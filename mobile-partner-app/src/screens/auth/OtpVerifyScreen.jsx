...
export default function OtpVerifyScreen({ route, navigation }) {
  ...
  async function handleVerify() {
    setError('');
    setBusy(true);
    const extra = purpose === 'SIGNUP' ? { role: 'PARTNER', name } : {};
  ...
  return (
    <SafeAreaView style={styles.screen}>
      <View style={styles.content}>
        ...
        {debugOtp && <Text style={styles.debugHint}>Demo mode — your code is {debugOtp}</Text>}
        {!!error && <Text style={styles.error}>{error}</Text>}

        <Button title={busy ? 'Verifying...' : 'Verify & continue'} onPress={handleVerify} disabled={busy} loading={busy} />
        <Text style={styles.back} onPress={() => navigation.goBack()}>Use a different number</Text>
      </View>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  title: { fontFamily: fonts.displayBold, fontSize: 22, color: colors.textPrimary, marginBottom: spacing(2) },
  ...
  debugHint: { fontFamily: fonts.bodyMedium, fontSize: 13, color: colors.accent, backgroundColor: colors.accentSoft, padding: spacing(3), borderRadius: radius.sm, marginBottom: spacing(4) },
